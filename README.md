...
- AlphaSeek-style hybrid backbone with:
...
python3 scripts/training.py --config alphaseek_hybrid_1d
```

Evaluate and backtest:

```bash
python3 scripts/evaluation.py --config alphaseek_hybrid_1d --run_dir runs/alphaseek_hybrid_1d/<run_name>
python3 scripts/simulate_trade.py --config alphaseek_hybrid_1d --run_dir runs/alphaseek_hybrid_1d/<run_name>
```
...
python3 scripts/simulate_trade.py \
  --config alphaseek_hybrid_1d \
  --run_dir runs/alphaseek_hybrid_1d/<run_name> \
...
bash cluster/grove/submit_grove_pipeline.sh alphaseek_hybrid_1d
```
...
python3 scripts/train_dqn.py \
  --config alphaseek_dqn_1d \
  --signal_config alphaseek_hybrid_1d \
  --signal_ckpt runs/alphaseek_hybrid_1d/<run_name>/checkpoints/last.ckpt
```
...
```bash
export CONDA_ENV=cryptomamba
bash cluster/grove/submit_grove_dqn.sh alphaseek_dqn_1d alphaseek_hybrid_1d /absolute/path/to/last.ckpt
```

## Key Configs

- Hybrid model: [configs/models/AlphaSeek/hybrid_v1.yaml](/Users/sribalaajinatarajankalaivendan/Desktop/AdaptConference/Mamba-TCN-Regnet/configs/models/AlphaSeek/hybrid_v1.yaml)
...
- RL config: [configs/rl/alphaseek_dqn_1d.yaml](/Users/sribalaajinatarajankalaivendan/Desktop/AdaptConference/Mamba-TCN-Regnet/configs/rl/alphaseek_dqn_1d.yaml)
