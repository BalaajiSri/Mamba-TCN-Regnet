# HybridSSM-Trader Paper Evidence
...
CONDA_ENV=cryptomamba bash experiments/paper_hybrid_ssm_trader/run_paper_matrix.sh
```

Current submitted jobs are under `runs/paper_hybrid_ssm_trader`. Dependent validation jobs run automatically after each training job succeeds.
...
CONDA_ENV=cryptomamba \
  experiments/paper_hybrid_ssm_trader/submit_dqn_from_run.sh \
  runs/paper_hybrid_ssm_trader/paper_hybrid_1d/<completed-run-dir>
```

This uses `configs/rl/hybrid_ssm_trader_dqn_paper_1d.yaml` and stores RL artifacts under `runs/paper_hybrid_ssm_trader_rl/`.
...
python3 scripts/paper_aggregate.py \
  ...
  --output experiments/paper_hybrid_ssm_trader/metrics/aggregate_metrics.json
```
